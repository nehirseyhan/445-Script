...
This document is meant to be spoken through in a grading session. It focuses on *logic and behavior*, not Python syntax.
...
- Owns:
  ...
  - Which trackers are subscribed to this item.
...
- **`update(**updates)`**
  ...
  - It uses a mapping of allowed fields to internal attribute names.
  - It ignores unknown fields and disallows empty values.
...
`CargoDirectory` is an in‑memory catalog of all items.
